...
def test_weather(mocker, client):
    with requests_mock.Mocker() as mock:
        mock.get('https://api.openweathermap.org/data/2.5/weather?q=London&appid=TEST&units=metric', json={
            'name': 'London',
            'main': {'temp': 10},
...
@pytest.mark.parametrize(('city', 'expected'), [
    ...
    ('New York', {'name': 'New York', 'temperature': 5, 'description': 'light snow'}),
])
def test_weather(city, expected, mocker, client):
    with requests_mock.Mocker() as mock:
        mock.get(f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid=TEST&units=metric', json={
            'name': city,
